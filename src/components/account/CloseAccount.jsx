...
const CloseAccount = (props) => {
  ...
  const DeleteAccount = async (user) => {
    setIsLoading(true)
    await dispatch(deleteAccount(user))
  ...
  return (
    <div className="closeWallet">
      <div>
        <div>
          <div className="login__header">Уверены, что хотите удалить аккаунт?</div>
          <div className="message">Восстановить данные будет невозможно. Проверьте баланс средств.</div>
      ...
      <div className="closeWallet__close_actions">
        ...
        <button
          ...
          onClick={() => {
            DeleteAccount(props.user)
          }}
        >
          {isLoading ? <ButtonLoader></ButtonLoader> : 'Удалить аккаунт'}
        </button>
      </div>
    </div>
  )
