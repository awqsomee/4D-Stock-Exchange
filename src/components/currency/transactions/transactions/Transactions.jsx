...
const Transactions = (props) => {
  ...
  const replenishHandler = async (dispatch, value, setValue) => {
    ...
    if (selectedCurrency?.symbol === 'RUB') {
      ...
      await dispatch(
        exchangeCurrency(selectedCurrency?.symbol, userCurrencies, transactions, value)
      )
      props.setmodalBoxDeposit(true)
    }
    setValue('')
  ...
  const sorting = (a, b) => {
    ...
    if (
      (value != '') &
      (value != 'Обмен валюты' || value != 'Пополнение баланса' || value != 'Вывод средств')
    ) {
      if (a[value] == null)
        if (sort) {
          if (0 > b[value]) {
  ...
  const sortedTransactions = useMemo(() => {
    return [...transactions]
      ?.filter((transaction) => {
        return (
          transaction?.symbol === selectedCurrency?.symbol ||
          transaction?.currency === selectedCurrency?.symbol
        )
      })
            ...
            <div className="transactions__difference">
              <div className="transactions__last_price">
                ...
                <div className="transactions__last_price__price">
                  {selectedCurrency?.latestPrice.toFixed(2)}
                </div>
              </div>
