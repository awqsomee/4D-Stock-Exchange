...
const NewWallet = (props) => {
  const [currency, setCurrency] = useState('Выберите валюту')
  const [isLoading, setIsLoading] = useState(true)
  const [modalBoxDeposit, setmodalBoxDeposit] = useState(false)
  const dispatch = useDispatch()
  const currecncies = Object.keys(store.getState().toolkit.currecncies)
  console.log('curr', currecncies)
        ...
        >
          {currecncies.map((currency) => (
            <option value={currency}>
              {store.getState().toolkit.currecncies[currency].name}
            </option>
          ))}
        </select>
