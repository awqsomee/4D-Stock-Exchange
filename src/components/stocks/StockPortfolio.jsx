...
const StockPortfolio = (props) => {
    ...
    const countChange = () => {
        if (props.stock?.prices.length > 0 && props.stock?.prices[0].close != null) {
            let count =
                ((props.stock?.prices[0].close - props.stock?.latestPrice) /
                    props.stock?.prices[0].close) *
                100
            setChanges(count)
        }
    }
    ...
    return (
        <div>
            <div className="stock">
                ...
                {changes < 0 ? (
                    <div className="stock__change stock__change__red">{changes.toFixed(2)}%</div>
                ) : (
                    <></>
                )}
                {changes == 0 ? <div className="stock__change">{changes.toFixed(2)}%</div> : <></>}
                {changes > 0 ? (
                    <div className="stock__change stock__change__green">{changes.toFixed(2)}%</div>
                ) : (
                    <></>
                )}
                ...
                <button
                    ...
                    onClick={() => {
                        ...
                        if (isAuth && counter > 0) {
                            if (props.buttonText == 'Купить')
                                dispatch(exchangeStocks(props.stock.symbol, counter))
                            if (props.buttonText == 'Продать') {
                                sellStocks()
                            }
                        }
                    }}
                >
                    {isLoading ? (
                        ...
                        `${(props.stock?.prices[0].close * counter).toFixed(2)} ${
